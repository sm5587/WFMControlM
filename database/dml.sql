...
INSERT OR REPLACE INTO "AppConfig" ("key", "value", "category", "label", "description", "isSecret", "updatedBy", "updatedAt") VALUES
  ...
  ('secrets.smtpTlsEnabled', 'false', 'SECRETS', 'SMTP TLS Enabled', 'Require TLS/STARTTLS for SMTP (ignored for local Mailpit on 127.0.0.1:1025)', 0, 'seed', CURRENT_TIMESTAMP),
  ('secrets.smtpTlsRejectUnauthorized', 'true', 'SECRETS', 'SMTP TLS Verify Certs', 'Validate SMTP server certificate when SMTP TLS Enabled is true', 0, 'seed', CURRENT_TIMESTAMP),
  ...
  ('infra.sshPendingFolderPath', '/mount/RWS4/batch_jobs/in', 'INFRA', 'Pending IN Folder', 'Remote path for pending upload files', 0, 'seed', CURRENT_TIMESTAMP),
  ('infra.sshRejectedUploadRoot', '/mount/RWS4/appuploads/upload', 'INFRA', 'Rejected Upload Root', 'Root path for rejected DTS file scan', 0, 'seed', CURRENT_TIMESTAMP),
  ('infra.sshCommandTimeoutSec', '30', 'INFRA', 'SSH Command Timeout (sec)', 'Timeout for pending IN folder find (maxdepth 1)', 0, 'seed', CURRENT_TIMESTAMP),
  ('infra.sshRejectedFindTimeoutSec', '120', 'INFRA', 'Rejected Find Timeout (sec)', 'Timeout for recursive rejected DTS find under upload root', 0, 'seed', CURRENT_TIMESTAMP),
  ('infra.db2ConnDir', '', 'INFRA', 'DB2 Conn Dir', 'Path to DB2 connection .txt files', 0, 'seed', CURRENT_TIMESTAMP),
  ('infra.db2LibDir', '', 'INFRA', 'DB2 Lib Dir', 'Path to DB2Connector.class & db2jcc4.jar', 0, 'seed', CURRENT_TIMESTAMP),
  ('infra.db2JavaPath', '', 'INFRA', 'Java Path', 'Path to java binary for DB2 JDBC connector (JDK 17+)', 0, 'seed', CURRENT_TIMESTAMP),
  ...
  ('infra.trustProxy', 'false', 'INFRA', 'Trust Proxy', 'Trust X-Forwarded-* headers from nginx/load balancer (enable in production behind TLS terminator)', 0, 'seed', CURRENT_TIMESTAMP),
  ('infra.requireHttps', 'false', 'INFRA', 'Require HTTPS', 'Redirect HTTP to HTTPS when behind a TLS-terminating reverse proxy', 0, 'seed', CURRENT_TIMESTAMP),
  ('infra.ssoEnabled', 'false', 'INFRA', 'SSO/LDAP Login Enabled', 'Enable corporate SSO/LDAP login via load-balancer email header. When false, only username/password login is available.', 0, 'seed', CURRENT_TIMESTAMP),
  ('infra.ssoEmailHeader', 'X-Forwarded-Email', 'INFRA', 'SSO Email Header', 'HTTP header name the load balancer sets with the authenticated user email', 0, 'seed', CURRENT_TIMESTAMP),
  ('infra.ssoAllowedDomain', 'zebra.com', 'INFRA', 'SSO Allowed Email Domain', 'Only @zebra.com (or configured domain) emails may register or sign in via SSO', 0, 'seed', CURRENT_TIMESTAMP),
  ...
  ('engine.db2ConnectorTimeoutMs', '120000', 'ENGINE', 'DB2 Connector Timeout (ms)', 'DB2 java child process timeout', 0, 'seed', CURRENT_TIMESTAMP),
  ('engine.db2ConnectorMaxBuffer', '10485760', 'ENGINE', 'DB2 Connector Max Buffer', 'DB2 java stdout max buffer size in bytes', 0, 'seed', CURRENT_TIMESTAMP),
