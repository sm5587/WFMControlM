...
class DB2DirectService {
  ...
  async getBatchStatusGrouped(clientId: string, days: number = configService.getInt('engine.batchQueryDays')): Promise<BatchJobGroup[]> {
    const sql = `SELECT JOB_TYPE, PLAN_TYPE, COUNT(*) AS CNT, ` +
      ...
      `SUM(CASE WHEN STATUS IN ('N','I') AND TIME_SUBMITTED IS NOT NULL AND TIME_SUBMITTED < CURRENT TIMESTAMP - ${configService.getInt('threshold.stalePendingDbMins')} MINUTES THEN 1 ELSE 0 END) AS STALE_PENDING, ` +
